import {
  ...
  export default function ModalPassword({ modal, setModal }) {
    ...
    let validate = (password) => {
      let errors = {};
      if (password === "" || password === null || password === undefined || password.length < 1) errors.password = "Debes ingresar una contraseña.";
      if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/.test(password)) errors.password = "Su contraseña debe incluir, al menos, 8 caracteres, una mayúscula, una minúscula y un número."
      return errors;
    };
              ...
              <View style={styles.containerButton}>
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    ...
                    dispatch(putPassword(password, token));
                    setErrors({password: "Ingrese una contraseña válida."});
                    setModal(false);
